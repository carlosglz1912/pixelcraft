...
interface GalleryState {
  ...
  addPending: (
    item: Omit<PendingMedia, "id" | "createdAt">,
    count?: number,
  ) => string[];
...
function inferOperation(
  model: string,
  prompt: string,
): "original" | "edit" | "upscale" | "remove-bg" {
  if (
    UPSCALE_PREFIXES.some((p) => model.startsWith(p)) ||
    prompt.includes("(upscaled)")
  )
    return "upscale";
  if (
    BG_REMOVAL_PREFIXES.some((p) => model.startsWith(p)) ||
    prompt.includes("(background removed)")
  )
    return "remove-bg";
  if (model.includes("/edit") || model.includes("image-to-image"))
    return "edit";
  return "original";
}
...
function sanitizeItemForPersistence(
  item: GeneratedMedia,
): GeneratedMedia | null {
  ...
  return {
    ...
    metadata: item.metadata
      ? {
          ...item.metadata,
          source: isEphemeralUrl(item.metadata.source)
            ? undefined
  ...
  setItem: (name, value) => {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "[gallery.persist] Local storage quota exceeded; skipping persistence update.",
          error,
        );
        return;
      }
    ...
    (set, get) => ({
      ...
      hydrateFromR2: async () => {
        ...
        try {
          ...
          const r2StorageIds = new Set<string>(
            r2Items.map((m: PersistedMedia) => m.storageId),
          );

          set((state) => {
            const localOnlyItems = state.items.filter((item) => {
              if (
                item.metadata?.storageId &&
                r2StorageIds.has(item.metadata.storageId)
              ) {
                return false;
              }
        ...
        void (async () => {
          for (const item of itemsToMigrate) {
            try {
              ...
              console.error(
                "[gallery] Failed to migrate item:",
                item.id,
                error,
              );
            }
      ...
      addPending: (item, count = 1) => {
        ...
        set((state) => ({
          pendingItems: [...nextPending, ...state.pendingItems],
        }));
          ...
          const result: PersistedMediaResult = await persistMedia({
            ...
            metadata: {
              ...
              duration: item.metadata?.duration
                ? parseFloat(item.metadata.duration)
                : undefined,
      ...
      getEditHistory: (id) => {
        ...
        for (let i = 0; i < ancestors.length; i++) {
          const op =
            i === 0
              ? "original"
              : inferOperation(ancestors[i].model, ancestors[i].prompt);
        ...
        const rootOp =
          ancestors.length > 0
            ? "original"
            : inferOperation(target.model, target.prompt);
        ...
        if (
          ancestors.length > 0 &&
          !result.some((r) => r.item.id === target.id)
        ) {
