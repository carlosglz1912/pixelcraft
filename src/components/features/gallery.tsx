...
function EstimatedCostBadge({ amount }: { amount?: number }) {
  ...
  return (
    <Badge
      variant="secondary"
      className="depth-secondary border border-primary/20 bg-primary/10 text-primary-tint"
    >
...
function FailedGalleryCard({ item }: { item: PendingMedia }) {
  ...
  function handleRetry() {
    ...
    toast.info(
      "Genera nuevamente con los mismos parámetros desde el panel lateral",
    );
  }
...
export function Gallery({ onSwitchTab }: GalleryProps) {
  ...
  const [upscaleModel, setUpscaleModel] = useState<UpscaleModelId>(
    "fal-ai/imageutils/super-resolution",
  );
  const [bgRemovalModel, setBgRemovalModel] = useState<BgRemovalModelId>(
    "fal-ai/imageutils/rembg",
  );
  const [loadingAction, setLoadingAction] = useState<
    "upscale" | "remove-bg" | null
  >(null);
  ...
  const migrateUnpersistedItems = useGallery(
    (state) => state.migrateUnpersistedItems,
  );
  ...
  const removeItemsFromCollection = useCollections(
    (state) => state.removeItems,
  );
  const getCollectionsForItem = useCollections(
    (state) => state.getCollectionsForItem,
  );
        ...
        while (failures < 5) {
          try {
            ...
            if (status.status === "COMPLETED") {
              ...
              if (!result) {
                failures += 1;
                console.info("[video.polling.result.pending]", {
                  requestId,
                  failures,
                });
            ...
            if (
              ...
              (status.status as string) === "CANCELLED"
            ) {
        ...
        <div className="surface-secondary flex items-center justify-between gap-3 border-b border-secondary/20 px-5 py-4">
          <div className="flex min-w-0 items-center gap-2">
            ...
            {isPersisting && (
              <Badge
                variant="secondary"
                className="depth-primary border border-green-500/20 bg-green-500/10 text-green-400"
              >
              ...
              <AnimatePresence mode="popLayout">
                {sortedItems.map((item, index) => (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`relative w-full bg-black/40 ${getMediaFrameClass(item.type, item.metadata)}`}
                      >
                        {item.type === "video" ? (
                          <video
                            src={item.url}
              ...
              <div className="min-w-0 border-b border-secondary/20 min-[1280px]:border-b-0 min-[1280px]:border-r">
                <DialogHeader className="surface-secondary border-b border-secondary/20 px-5 py-4">
                  ...
                  <DialogDescription className="sr-only">
                    Vista previa del recurso seleccionado y acciones disponibles
                    para editarlo o abrirlo.
                  </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        {getDisplayModel(
                          selected.model,
                          selected.type,
                          selected.metadata?.videoMode,
                        )}
                      </Badge>
                      <CostBadge tier={selected.costTier} />
                      <EstimatedCostBadge
                        amount={selected.metadata?.estimatedCost}
                      />
                      <Badge
                        variant="secondary"
                        className="depth-primary border border-primary/20 bg-primary/10 text-primary-tint"
                  ...
                  <div className="space-y-4 p-4">
                    <div className="depth-primary rounded-3xl border border-primary/20 bg-primary/10 p-4">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm font-semibold text-white">
                          Editar en conversación
                        </p>
                      </div>
                      <p className="mt-2 text-sm text-slate-200">
                        Abre una ruta dedicada para iterar ediciones sobre esta
                        imagen.
                      </p>
                      <Button
                    ...
                    <div className="depth-secondary rounded-3xl border border-secondary/20 bg-slate-900/70 p-4">
                      ...
                      <Select
                        value={upscaleModel}
                        onValueChange={(value) =>
                          setUpscaleModel(value as UpscaleModelId)
                        }
                      >
                        ...
                        <SelectContent>
                          {Object.entries(UPSCALE_MODELS).map(
                            ([id, config]) => (
                    ...
                    <div className="depth-secondary rounded-3xl border border-secondary/20 bg-slate-900/70 p-4">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setBgRemovalModel(value as BgRemovalModelId)
                        }
                      >
                        <SelectTrigger className="depth-secondary mt-3 border border-secondary/20 bg-slate-900/70 text-white">
                          <SelectValue />
                        ...
                        <SelectContent>
                          {Object.entries(BG_REMOVAL_MODELS).map(
                            ([id, config]) => (
                              <SelectItem key={id} value={id}>
                                {config.name}
                              </SelectItem>
                    ...
                    {!canTransform ? (
                      <div className="depth-secondary rounded-3xl border border-secondary/20 bg-secondary/10 p-4 text-sm text-slate-200">
                        Las acciones de edición solo están disponibles para
                        imágenes.
                      </div>
                    ) : null}
                    ...
                    {selected &&
                      (() => {
                        const itemCollections = getCollectionsForItem(
                          selected.id,
                        );
                        return (
                          <div className="depth-secondary rounded-3xl border border-secondary/20 bg-slate-900/70 p-4">
                ...
                {collections.map((collection) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`h-2.5 w-2.5 shrink-0 rounded-full ${colorConfig.dot}`}
                      />
                      ...
                      <Badge
                        variant="secondary"
                        className="depth-secondary border border-secondary/20 bg-secondary/10 text-xs text-secondary-tint"
                      >
                      ...
                      <input
                        ...
                        onChange={() => {
                          if (isAssigned) {
                            removeItemsFromCollection(collection.id, [
                              selected.id,
                            ]);
